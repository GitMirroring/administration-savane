...
EOD;
...
else
 {
    ...
    $sys_dbhost = $sys_dbname = $sys_dbuser = $sys_dbpasswd = "";
    ...
    if (
      empty ($sys_dbhost) && empty ($sys_dbname)
        && empty ($sys_dbuser) && empty ($sys_dbpasswd)
    )
      {
        print $first_run_message;
        exit (0);
    ...
    foreach (['host', 'name', 'user', 'passwd'] as $v)
      if (empty (${"sys_db$v"}))
        die ("savannah-dev-error: '\$sys_db$v' is not set in '$db_file'");
  }
